...
spark = SparkSession.builder.appName("spatil62-minio-read-part2-t").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
csvdf = spark.read.csv('s3a://spatil62/30-newcsv', header=True, schema=schema_value)

dataf = spark.read.csv('s3a://spatil62/30-newcsv', header=True, schema=schema_value)
dataf.write.mode("overwrite").option("compression", "none").json("s3a://spatil62/30-rewriteJSON")
jsondf = spark.read.json('s3a://spatil62/30-rewriteJSON')

dataf.write.mode("overwrite").option("compression", "none").parquet("s3a://spatil62/30-rewriteParquet")
parquetdf = spark.read.parquet('s3a://spatil62/30-rewriteParquet')
...
# MARIA DB CONNECTION 
datadf = spark.read.csv('s3a://itmd521/30.txt', header=True, schema=schema_value)
datadf.write.format("jdbc").option("url","jdbc:mysql://database-240-vm0.service.consul:3306/ncdc").option("driver","com.mysql.cj.jdbc.Driver").option("dbtable","spatil62Thirties").option("user",os.getenv("MYSQLUSER")).option("truncate",True).mode("overwrite").option("password", os.getenv("MYSQLPASS")).save()
newDatadF = spark.read.format("jdbc").option("url","jdbc:mysql://database-240-vm0.service.consul:3306/ncdc").option("driver","com.mysql.cj.jdbc.Driver").option("dbtable","spatil62Thirties").option("user",os.getenv("MYSQLUSER")).option("truncate",True).option("password", os.getenv("MYSQLPASS")).load()
