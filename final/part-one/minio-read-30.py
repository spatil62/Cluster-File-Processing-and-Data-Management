...
spark = SparkSession.builder.appName("spatil62-minio-read-and-process-30").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()
...
splitDF.write.format("overwrite").csv("s3a://spatil62/30-newcsv")
splitDF.write.format("overwrite").parquet("s3a://spatil62/30-newparquet")
splitDF.write.format("overwrite").json("s3a://spatil62/30-newjson")
splitDF.write.format("overwrite").option("compression", "lz4").csv("s3a://spatil62/30-lz4com")
writenewDF = splitDF.coalesce(1)
writenewDF.write.format("overwrite").csv("s3a://spatil62/30-singlepart")
