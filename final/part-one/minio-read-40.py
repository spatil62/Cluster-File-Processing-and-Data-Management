...
spark = SparkSession.builder.appName("spatil62-minio-read-40").config('spark.driver.host','spark-edge-vm0.service.consul').config(conf=conf).getOrCreate()

# Read the datatype into a DataFrame
df = spark.read.csv('s3:40.txt')
...
splitDF.write.csv("s3:40-csv")
splitDF.write.parquet("s3:40-parquet")
